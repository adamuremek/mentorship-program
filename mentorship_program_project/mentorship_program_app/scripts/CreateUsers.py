...
def run():
    ...
    list_of_users = []
    #list_of_users.append(create_user( "fryan3@svsu.edu", "Qwerty123!@#", User.Role.MENTEE, datetime.now(), True, False, "Frank", "Ryan", "(256) 365-1024", datetime(2000,9,27), "Male", "Him/He"))
    # Create admin
    list_of_users.append(create_user('admin@admin.com', 'Qwerty123!@#', User.Role.ADMIN, datetime.now(),True, False, 'Scott', 'James', '999-888-7777', datetime(1980, 5, 17), 'Male', 'He/Him'))
    # Create mentors
   ...
   #list_of_users.append(create_user('EmilyJohnson@gmail.com', 'Qwerty123!@#', User.Role.MENTOR, datetime.now(), True, False, 'Emily', 'Johnson', '555-123-4567', datetime(1994, 6, 20), 'Female', 'She/Her'))
    ...
    for user in list_of_users:
        ...
        if User.Role.ADMIN :
            admin = SuperAdminEntry.objects.create(
                user_account = user
            )
            admin.save()
