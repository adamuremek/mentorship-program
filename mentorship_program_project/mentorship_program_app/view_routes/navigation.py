...
from django.db.models import Count, Q, Case,When, F, BooleanField, Value
...
def dashboard(req):
    ...
    if opposite_role == "Mentor":
        mentor_data = Mentor.objects.annotate(
        num_mentees=Count('_mentee_set', distinct=True)
    ).annotate(
        has_maxed_mentees=Case(
            When(num_mentees__gte=F('int_max_mentees'), then=Value(True)),
            default=Value(False),
        ...
        # Now, we ensure that these mentors are only those linked to a User with the 'Mentor' role
        # And perform the necessary prefetching and selecting related objects
        card_data = mentor_data.filter(
            account__str_role='Mentor'  # Filter to include only mentors where the linked user has the 'Mentor' role
        ).select_related(
            'account'  # Link each Mentor to the corresponding User account
        ).prefetch_related(
            'account__interests'  # Prefetch the interests of the associated User
        )
        users = [user.account.sanitize_black_properties() for user in card_data]
    ...
    print("Time for query: ", time.time()-start_time )
