...
const mentee_bars = mentee_bar_container.querySelectorAll(
    ".mentee_management_bar_container_active",
);
const mentor_bars = mentor_bar_container.querySelectorAll(
    ".mentor_management_bar_container_active",
);

// Determine static page elements
const save_button = document.getElementById("save_button");
const cancel_button = document.getElementById("cancel_button");
const add_new_organization_button = document.getElementById(
    "add_new_organization_button",
);
const filter_user_button = document.querySelector("#filter_users_button");
const filter_organization_button = document.querySelector(
    "#filter_organization_button",
);
const user_search_bar = document.querySelector("#user_search_bar");
const organization_search_bar = document.querySelector(
    "#organization_search_bar",
);
const session_user_admin_flag = document.querySelector(
    "#session_user_admin_flag",
);
const user_management_message = document.querySelector(
    ".user_management_message_bar_inactive",
);
const loading_overloy = document.querySelector("#loading-overlay");

// Determine and store organzition elements
let organization_bars = mentor_bar_container.querySelectorAll(
    ".organization_management_bar_container_active",
);

const non_organization_bars = document.querySelectorAll(
    "#mentor_bar_container > .mentor_management_bar_container_active",
);

const create_organization_button = document.getElementById(
    "create_organization_button",
);
const exit_add_new_organization_button = document.getElementById(
    "exit_add_new_organization_button",
);
const add_new_organization_modal = document.querySelector(
    "#organization-modal",
);
const add_new_organization_message_bar = add_new_organization_modal
    .querySelector(".create_organization_message_bar_inactive");
const add_new_organization_name = add_new_organization_modal.querySelector(
    ".user-input",
);
...
// All mentor bars without an organization attached to them
export function determine_all_non_organization_bars() {
    return non_organization_bars;
}
...
export function determine_user_name(passed_bar) {
    return passed_bar.querySelector(".user_management_bar_name").innerHTML.trim()
        .toLowerCase();
}

// Returns organization value from passed bar
export function determine_organization_name_value(passed_bar) {
    return passed_bar.querySelector(".organization_management_bar_name").innerHTML
        .trim();
}
...
export function determine_disabled_value(passed_bar) {
    return Number(
        passed_bar.querySelector("#user_account_disabled").textContent.trim(),
    );
}
...
export function determine_parent_organization_bar_element(passed_bar) {
    ...
    for (
        let organiation_index = 0;
        organiation_index < organization_bars.length;
        organiation_index++
    ) {
        ...
        let admin_list = determine_organization_admin_list(
            organization_bars[organiation_index],
        );
        let mentor_list = determine_organization_mentor_list(
            organization_bars[organiation_index],
        );
...
export function return_updated_mentor_list() {
    ...
    mentor_bars.forEach((mentor_bar) => {
        ...
        if (
            current_mentees < max_mentees && !determine_disabled_value(mentor_bar)
        ) {
...
export function determine_if_bar_organization_admin(
    organitization_bar,
    user_bar,
) {
    ...
    const organitization_admin_list = determine_organization_admin_list(
        organitization_bar,
    );
...
export function determine_if_bars_valid_transfer(
    organization_bar,
    passed_bar_1,
    passed_bar_2,
) {
    ...
    return ((determine_if_bar_organization_admin(
        organization_bar,
        passed_bar_1,
    ) && !determine_if_bar_organization_admin(organization_bar, passed_bar_2)) ||
        (!determine_if_bar_organization_admin(organization_bar, passed_bar_1) &&
            determine_if_bar_organization_admin(organization_bar, passed_bar_2)));
}

// Returns value based on if the passed mentor bar's organization bar and the passed organization bars match
export function determine_if_bars_valid_edit_organzation(
    organitization_bar,
    mentor_bar,
) {
    ...
    const prev_organitization_bar = determine_parent_organization_bar_element(
        mentor_bar,
    );
    ...
    if ((prev_organitization_bar != null)) {
        ...
        if (
            determine_organization_id(prev_organitization_bar) !=
            determine_organization_id(organitization_bar)
        ) {
            return_value = true;
        }
    } else {
...
export function determine_if_organization_name_unique(new_organization_name) {
    ...
    for (let index = 0; index < all_roganization_bars.length; index++) {
        ...
        let current_organization_name = determine_organization_name_value(
            all_roganization_bars[index],
        ).toLowerCase();
...
export function deteremine_and_refresh_all_organization_bars() {
    organization_bars = mentor_bar_container.querySelectorAll(
        ".organization_management_bar_container_active",
    );
}
...
export function determine_if_word_button(passsed_button) {
    ...
    return (passsed_button.classList.contains(
        "admin_user_management_word_button_active",
    ) ||
        passsed_button.classList.contains(
            "admin_user_management_word_button_inactive",
        ));
}
...
export function deteremine_if_button(passed_button) {
    ...
    return (passed_button.classList.contains(
        "admin_user_management_button_clear_active",
    ) ||
        passed_button.classList.contains(
            "admin_user_management_button_clear_inactive",
        ));
}
...
export function determine_if_user_mentor_bar(passed_bar) {
    ...
    return (passed_bar.classList.contains(
        "mentor_management_bar_container_active",
    ) ||
        passed_bar.classList.contains("mentor_management_bar_container_inactive"));
}
...
export function determine_if_user_mentee_bar(passed_bar) {
    ...
    return (passed_bar.classList.contains(
        ...
        passed_bar.classList.contains("mentee_management_bar_container_inactive"));
}
...
export function deteremine_if_organization(passed_bar) {
    ...
    return (passed_bar.classList.contains(
        ...
        passed_bar.classList.contains(
            "organization_management_bar_container_inactive",
        ));
}
...
export function deteremine_if_create_organization_message_bar(passed_bar) {
    ...
    return (passed_bar.classList.contains(
        "create_organization_message_bar_active",
    ) ||
        passed_bar.classList.contains("create_organization_message_bar_inactive"));
}
...
export function deteremine_if_user_maanagement_message_bar(passed_bar) {
    // Checks passed bar's class list and check if it has as active or inactive class for create organization message bar
    return (passed_bar.classList.contains("user_management_message_bar_active") ||
        passed_bar.classList.contains("user_management_message_bar_inactive"));
}
...
export function deteremine_if_event_toggle(
    prev_event,
    new_event_type,
    new_event_id,
) {
...
export function determine_if_mentor_included_in_passed_bars(
    valid_mentor_bars,
    mentor_bar,
) {
