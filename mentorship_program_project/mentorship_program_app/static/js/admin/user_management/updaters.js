...
export function show_add_new_organization_modal() {
  ...
  const add_new_organization_modal = determiners
    .determine_add_new_organization_modal();
...
export function update_remove_from_organization(mentor_bar) {
  ...
  update_not_show(
    determiners.determine_promote_organization_button(mentor_bar),
  );
...
export function update_add_to_organization(organization_bar, mentor_bar) {
  ...
  determiners.determine_organization_mentor_list(organization_bar).appendChild(
    mentor_bar,
  );
...
export function update_promote_organization_admin(
  organization_bar,
  mentor_bar,
) {
  ...
  determiners.determine_organization_admin_list(organization_bar).appendChild(
    mentor_bar,
  );
...
export function update_demote_organization_admin(organization_bar) {
  ...
  const current_admins = determiners.determine_mentor_bars(
    determiners.determine_organization_admin_list(organization_bar),
  );
  ...
  if (current_admins != null) {
    ...
    current_admins.forEach((current_admin) => {
      ...
      determiners.determine_organization_mentor_list(organization_bar)
        .appendChild(current_admin);

      // Determine and update to have promote organization admin button
      update_show(
        determiners.determine_promote_organization_button(current_admin),
      );
    });
  }
}
...
export function update_decerment_mentor_mentees(user_bar, mentee_id) {
  ...
  determiners.determine_current_mentees(user_bar).innerHTML =
    determiners.determine_current_mentees_value(user_bar) - 1;

  // Determine mentee values and create array of mentee values from the string
  let updated_mentee_values = determiners.return_array_from_string(
    determiners.determine_mentees_value(user_bar),
  );
...
export function update_incerment_mentor_mentees(user_bar, mentee_id) {
  ...
  const updated_mentee_values = determiners.return_array_from_string(
    determiners.determine_mentees_value(user_bar),
  );
...
export function update_reable_bar(user_bar) {
  ...
  update_buttons_toggle_on(
    determiners.determine_disable_button(user_bar),
...
export function update_buttons_toggle_on(passed_button_1, passed_button_2) {
  ...
  passed_button_1.classList.remove(
...
export function update_buttons_toggle_off(passed_button_1, passed_button_2) {
  ...
  passed_button_2.classList.remove(
    "admin_user_management_button_clear_inactive",
  );
...
export function update_all_organization_admin_bars() {
  ...
  organization_bars.forEach((organization_bar) => {
    ...
    determiners.determine_mentor_bars(
      determiners.determine_organization_admin_list(organization_bar),
    ).forEach((admin_bar) => {
      // Determine and hide promote organization button
      update_not_show(
        determiners.determine_promote_organization_button(admin_bar),
      );
    });
  });
}
...
export function update_filter_organization_bars(user_input) {
  ...
  // Filter out ALL bars without an organization attached to them
  const non_organization_bars = determiners
    .determine_all_non_organization_bars();

  non_organization_bars.forEach((non_organization_bar) => {
    update_not_show(non_organization_bar);
  });

  // Cycle through organization bars
  organization_bars.forEach((organization_bar) => {
    // Determine orgnaization bar name
    current_organization_name = determiners.determine_organization_name_value(
      organization_bar,
    ).toLowerCase();
...
export function update_create_organization(
  organitization_name,
  organization_account_string,
  remove_organization_event,
  organization_clicked_event,
) {
  ...
  const session_user_admin_flag = determiners
    .determine_session_user_admin_flag();

  // Create organization management bar container element
  const organization_management_bar_container_active = document.createElement(
    "div",
  );
  organization_management_bar_container_active.classList.add(
  ...
  organization_management_bar_container_active.appendChild(
    organization_management_bar,
  );
  ...
  organization_management_bar_name.classList.add(
    "organization_management_bar_name",
  );
  ...
  if (session_user_admin_flag) {
    ...
    remove_organization_button.classList.add(
    ...
    remove_organization_button.addEventListener("click", function () {
      remove_organization_event(organization_management_bar_container_active);
    });
  }
  ...
  // Attach organization mangement bar to mentor bar container
  mentor_bar_container.appendChild(
    organization_management_bar_container_active,
  );

  // Attach listener for organization clicked
  organization_management_bar_container_active.addEventListener(
    "click",
    function () {
      organization_clicked_event(organization_management_bar_container_active);
    },
  );
}
